...
export default function BookConsultantPage() {
  ...
  const [showForm, setShowForm] = useState(
    initialName || initialPhone || initialEmail ? true : false,
  );

  useEffect(() => {
    if (paymentStatus === "success") {
      trackPurchase(59.0, "INR", [{ item_name: "Consultation" }]);
    }
  }, [paymentStatus]);
  ...
  const initiatePayment = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(payload.error || "Failed to fetch payload");
  ...
  const includedItems = [
    {
      title: "Custom Project Blueprint",
      desc: "Tailored strategy based on your target investment, location, and climate.",
      icon: MapPin,
    },
    {
      title: "Subsidy & Loan Guidance",
      desc: "Expert insights on government schemes, NHB/NABARD subsidies, and documentation.",
      icon: Briefcase,
    },
    {
      title: "Vendor & Spawn Sourcing",
      desc: "Direct connections to verified high-yield spawn suppliers and raw material vendors.",
      icon: Layers,
    },
    {
      title: "Contamination Troubleshooting",
      desc: "Quick diagnostic frameworks to eliminate mold, pests, and yield drops.",
      icon: ShieldCheck,
    },
  ];
    ...
    <div className="relative pb-24 md:pb-0 pt-16 md:pt-32 overflow-hidden min-h-screen selection:bg-blue-500/30">
      ...
      <div className="py-4 md:py-16 text-center px-2 md:px-4">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="badge mx-auto mb-2 md:mb-6 text-[9px] md:text-sm bg-blue-500/10 text-blue-400 border-blue-500/20 px-2 py-0.5 md:px-4 md:py-1">
              Premium Consulting
            </div>
            ...
            <p className="dark:text-slate-400 text-slate-600 text-[10px] md:text-xl max-w-2xl mx-auto font-medium mb-3 md:mb-10 leading-snug px-2">
              Get direct, actionable solutions from industry experts to avoid
              costly mistakes and scale profitably.
            </p>
            ...
            {!showForm ? (
              ...
              >
                <Zap size={14} className="md:w-6 md:h-6" /> Book Consultation
                Now - ₹59
              </motion.button>
            ) : (
              <motion.form
              ...
              >
                <div>
                  <label className="text-xs md:text-sm font-bold dark:text-slate-300 text-slate-700 ml-1 mb-1 block">
                    Full Name
                  </label>
                  <input
                    type="text"
      ...
      <div className="py-4 md:py-16 dark:bg-white/5 bg-black/5 px-2 md:px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-4 md:mb-12">
            <h3 className="text-md md:text-3xl font-bold dark:text-white text-slate-900 uppercase tracking-tight mb-0.5 md:mb-2">
              What's <span className="text-blue-500">Included</span>
            </h3>
            <p className="dark:text-slate-400 text-slate-600 text-[9px] md:text-base">
              Everything you need to launch and scale efficiently.
            </p>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-1.5 md:gap-6">
            {includedItems.map((item, i) => (
              ...
              >
                ...
                <h4 className="text-[10px] md:text-xl font-bold dark:text-white text-slate-900 mb-0.5 md:mb-3 leading-tight">
                  {item.title}
                </h4>
      ...
      <div className="py-4 md:py-16 px-2 md:px-4">
        <div className="max-w-4xl mx-auto glass p-3 md:p-10 rounded-xl md:rounded-[3rem] border dark:border-white/10 border-black/10 relative overflow-hidden">
          ...
          <h3 className="text-md md:text-3xl font-bold dark:text-white text-slate-900 uppercase tracking-tight mb-2 md:mb-8 text-center relative z-10">
          ...
          <ul className="space-y-1.5 md:space-y-6 relative z-10 max-w-2xl mx-auto">
            {whoIsItFor.map((item, i) => (
              <li
                key={i}
                className="flex items-center gap-1.5 md:gap-4 dark:text-slate-200 text-slate-800 font-bold text-[9px] md:text-lg text-left"
              >
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-400 mb-6">
                Your consultation booking was not completed.
                <br />
              ...
              {!whatsappSubmitted ? (
                <>
                  ...
                  <p className="text-xs md:text-sm text-slate-400 mb-6">
                    Please confirm your details to connect with our consultant.
                  </p>
                  ...
                  >
                    <div>
                      <label className="block text-[10px] md:text-xs font-bold text-slate-300 mb-1.5 uppercase tracking-wider">
                        Full Name
                      </label>
                      <input
                        type="text"

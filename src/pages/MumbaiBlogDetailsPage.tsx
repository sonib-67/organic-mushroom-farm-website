...
import { getMumbaiBlogByPath } from "../data/mumbaiBlogsData";
import { SHIPPING_DETAILS, MERCHANT_RETURN_POLICY } from "../utils/seoSchemas";

export default function MumbaiBlogDetailsPage() {
  const { blogRoute } = useParams();
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(null);

  if (!blogRoute) {
    return <Navigate to="/cities/maharashtra/mumbai" replace />;
  }

  // Look up the exact blog record
  const blog = getMumbaiBlogByPath(blogRoute);
  ...
  const reviewCounts = [
    118, 143, 85, 172, 130, 96, 164, 121, 107, 155, 189, 74,
  ];
  ...
  const jsonLdSchema = {
    ...
    "@type": "Product",
    id: `https://organicmushroomfarm.shop/locations/mumbai/${blog.path}#product`,
    name: blog.title,
  ...
  return (
    <>
      <SEO
        title={blog.title}
        description={blog.meta}
        keywords={`${blog.keyword}, mumbai mushroom training, how to grow mushroom in maharashtra`}
      />
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-12">
            <div className="flex flex-wrap items-center gap-3 mb-6">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full dark:bg-primary-start/10 bg-black/10 text-primary-start text-[10px] font-black uppercase tracking-wider border border-primary-start/20">
                <MapPin size={10} className="animate-pulse" /> Region: Mumbai
              </span>
          ...
          <div className="glass p-8 md:p-12 rounded-[2.5rem] border dark:border-white/5 border-black/5 mb-12">
            ...
            <div className="prose max-w-none dark:prose-invert">
              <p>
                Mumbai is one of India's largest food markets. Every day
                thousands of kilograms of fresh mushrooms are supplied to
                restaurants, hotels, supermarkets and online grocery stores.
              </p>
              <p>
                Because of increasing awareness about healthy food, demand is
                growing for:
              </p>
              <ul>
              ...
              <p>
                Medicinal mushrooms are also becoming popular among
              ...
              <p>
                Whether you want a small home-based farm or a commercial
                mushroom production unit, proper planning helps reduce
                production risks.
              </p>

              <h3>Online Mushroom Training</h3>
              <p>
                घरबसल्या संपूर्ण मशरूम शेती शिकण्यासाठी ऑनलाइन प्रशिक्षण उपलब्ध
                आहे.
              </p>
              ...
              <p>
                Students from Mumbai, Navi Mumbai, Thane, Panvel, Kalyan,
                Dombivli and nearby villages can join online training.
              </p>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mt-10">
              <div className="p-5 rounded-2xl dark:bg-white/[0.02] bg-black/[0.02] border dark:border-white/5 border-black/10">
                <h4 className="dark:text-white text-slate-950 font-bold text-sm mb-1 flex items-center gap-1.5">
                  <Sprout size={14} className="text-primary-start" /> F1 Mother
                  Spawns
                </h4>
                <p className="dark:text-slate-400 text-slate-600 text-xs leading-relaxed font-semibold">
                  Source sterile F1-grade mother seeds developed locally in
                  cleanroom environments to maximize yield.
                </p>
              </div>
              <div className="p-5 rounded-2xl dark:bg-white/[0.02] bg-black/[0.02] border dark:border-white/5 border-black/10">
                <h4 className="dark:text-white text-slate-950 font-bold text-sm mb-1 flex items-center gap-1.5">
                  <ShieldCheck size={14} className="text-green-500" /> Buyback
                  Guarantee
                </h4>
                <p className="dark:text-slate-400 text-slate-600 text-xs leading-relaxed font-semibold">
                  Support linkages connected straight with major mandis, drying
                  hubs, and exporter warehouses.
                </p>
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    <ChevronDown
                      ...
                      className={`text-slate-400 shrink-0 transition-transform duration-200 ${
                        openFaqIndex === index
                          ? "rotate-180 dark:text-white text-slate-900"
                          : ""
                      }`}
                    />
                  </button>
          ...
          <div className="glass p-8 md:p-12 rounded-[2.5rem] border dark:border-white/10 border-black/10 text-center relative overflow-hidden">
            ...
            <h3 className="text-2xl font-extrabold dark:text-white text-slate-950 mb-4">
              Ready to Setup Your Commercial Unit?
            </h3>
            <p className="dark:text-slate-400 text-slate-600 text-sm max-w-lg mx-auto mb-8 font-semibold">
              Connect directly with our master consultancy network today to
              access blueprints, legal templates, and subsidy documentations.
            </p>

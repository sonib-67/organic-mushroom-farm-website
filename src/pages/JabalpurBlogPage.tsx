...
export default function JabalpurBlogPage() {
  const { slug } = useParams<{ slug: string }>();
  
  const blog = jabalpurBlogs.find((b) => b.slug === slug);
    ...
    <div className="pt-24 pb-16 min-h-screen bg-slate-50 dark:bg-[#09090b]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
          ...
          <ChevronRight size={14} />
          <Link to="/locations/jabalpur" className="hover:text-brand-blue transition-colors">Jabalpur</Link>
          <ChevronRight size={14} />
        ...
        >
          <ArrowLeft size={16} /> Back to Jabalpur Resources
        </Link>

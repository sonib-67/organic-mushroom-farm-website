...
const newFunc = `async function generateInvoicePDF(payload: any): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Clean divider
      doc.moveTo(50, 160).lineTo(545, 160).lineWidth(0.5).strokeColor('#e5e7eb').stroke();

      // Billed To Section
      doc.fillColor(textLight).fontSize(9).font('Helvetica-Bold').text('BILLED TO', 50, 180, { characterSpacing: 1.5 });
      doc.fillColor(textDark).fontSize(14).font('Helvetica-Bold').text(payload.name || 'Customer', 50, 195);
      
      doc.fillColor(textMuted).fontSize(10).font('Helvetica')
         .text(payload.email || '', 50, 215)
         .text('Phone: ' + (payload.phone || 'N/A'), 50, 230);

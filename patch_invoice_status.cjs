  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor(textDark).fontSize(10).font('Helvetica-Bold')
         .text('Invoice No:', 350, 95)
         .text('Invoice Date:', 350, 110)
         .text('Payment Mode:', 350, 125)
         .text('Due Date:', 350, 140)
         .text('Status:', 350, 155);
         
      doc.fillColor(textMuted).font('Helvetica')
         .text(invoiceNo, 450, 95, { align: 'right', width: 95 })
         .text(invoiceDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }), 450, 110, { align: 'right', width: 95 })
         .text(payload.paymentMode || 'Online Payment', 450, 125, { align: 'right', width: 95 })
         .text(dueDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }), 450, 140, { align: 'right', width: 95 });
         
      doc.fillColor(statusColor).font('Helvetica-Bold')
         .text(statusText, 450, 155, { align: 'right', width: 95 });

      // Clean divider
      doc.moveTo(50, 175).lineTo(545, 175).lineWidth(0.5).strokeColor('#e5e7eb').stroke();

      // Billed To Section
      doc.fillColor(textLight).fontSize(9).font('Helvetica-Bold').text('BILLED TO', 50, 195, { characterSpacing: 1.5 });
      doc.fillColor(textDark).fontSize(14).font('Helvetica-Bold').text(payload.name || 'Customer', 50, 210);
      
      doc.fillColor(textMuted).fontSize(10).font('Helvetica')
         .text(payload.email || '', 50, 230)
         .text('Phone: ' + (payload.phone || 'N/A'), 50, 245);
      ...
      doc.fillColor(textDark).font('Helvetica').fontSize(11)
         ...
         .text('Rs. ' + (payload.amount || '0'), 0, rowTop, { align: 'right', width: 545 });
      ...
      doc.fillColor(accent).fontSize(18)
         .text('Rs. ' + (payload.amount || '0'), 0, totalTop - 2, { align: 'right', width: 545 });

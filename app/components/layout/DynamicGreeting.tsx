...
export const DynamicGreeting: React.FC = () => {
    ...
    const fetchLocationAndWeather = async () => {
      try {
        ...
        try {
          ...
          try {
            ...
            if (fbData.latitude && fbData.longitude) {
              ...
              country = fbData.country_name || 'India';
            }
          } catch {
            lat = 23.1815;
            lon = 79.9864;
        ...
        setWeather({
          ...
          uvIndex: wData.current?.uv_index ?? 3,
          rain: wData.current?.precipitation ?? 0,
          cloudCover: wData.current?.cloud_cover ?? 10,
  ...
  const rawSlides: Array<{ id: string; content: React.ReactNode } | null> = weather
    ? [
        {
          id: 'welcome',
          content: (
        ...
        weather.dewPoint !== undefined
          ? {
              ...
              content: (
                <>
                  Dew Point: {weather.dewPoint}°C <span className="animate-pulse inline-block">🌫️</span>
                </>
              ),
            }
          : null,
        weather.windSpeed !== undefined
          ? {
              ...
              content: (
                <>
                  Wind Speed: {weather.windSpeed} km/h <span className="animate-pulse inline-block">💨</span>
                </>
              ),
            }
          : null,
        weather.uvIndex !== undefined
          ? {
              ...
              content: (
                <>
                  UV Index: {weather.uvIndex} <span className="animate-pulse inline-block">☀️</span>
                </>
              ),
            }
          : null,
        weather.rain !== undefined
          ? {
              ...
              content: (
                <>
                  Rain: {weather.rain} mm <span className="animate-pulse inline-block">🌧️</span>
                </>
              ),
            }
          : null,
        weather.cloudCover !== undefined
          ? {
              ...
              content: (
                <>
                  Cloud Cover: {weather.cloudCover}% <span className="animate-pulse inline-block">☁️</span>
                </>
              ),
            }
          : null,
        weather.airPressure !== undefined
          ? {
              id: 'pressure',
              content: (
                <>
                  Air Pressure: {weather.airPressure} hPa <span className="animate-pulse inline-block">📉</span>
                </>
              ),
            }
          : null,
        {
          id: 'suggestion',
          content: <>{weather.temp !== undefined ? getSuggestion(weather.temp) : ''}</>,
        },
        {
          id: 'weather',
          content: (
            <>
              {weather.locationStr}: {weather.temp}°C, Hum {weather.humidity}%{' '}
